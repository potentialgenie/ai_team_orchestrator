-- 🎯 COMPLETE GOAL-DRIVEN SYSTEM DATABASE SETUP (FIXED VERSION)
...
CREATE INDEX IF NOT EXISTS idx_workspace_goals_priority ON workspace_goals(priority DESC);
CREATE INDEX IF NOT EXISTS idx_workspace_goals_metric_type ON workspace_goals(metric_type);
CREATE INDEX IF NOT EXISTS idx_workspace_goals_validation ON workspace_goals(last_validation_at, status) WHERE status = 'active';

-- Add RLS for security
ALTER TABLE workspace_goals ENABLE ROW LEVEL SECURITY;

-- Create update timestamp trigger
DROP TRIGGER IF EXISTS trigger_workspace_goals_timestamp ON workspace_goals;
DROP FUNCTION IF EXISTS update_workspace_goals_timestamp();
...
-- ============================================
-- 2️⃣ WORKSPACE INSIGHTS TABLE (Memory System) - Skip if exists
-- ============================================

-- Check if table exists before creating
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'workspace_insights') THEN
...
-- Indexes for workspace_insights (all with IF NOT EXISTS)
CREATE INDEX IF NOT EXISTS idx_workspace_insights_workspace_id ON workspace_insights(workspace_id);
CREATE INDEX IF NOT EXISTS idx_workspace_insights_task_id ON workspace_insights(task_id) WHERE task_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_workspace_insights_type ON workspace_insights(insight_type);
...
CREATE INDEX IF NOT EXISTS idx_workspace_insights_created ON workspace_insights(created_at DESC);

-- Enable RLS
ALTER TABLE workspace_insights ENABLE ROW LEVEL SECURITY;

-- Anti-pollution: Limit insights per workspace
DROP TRIGGER IF EXISTS enforce_workspace_insight_limit ON workspace_insights;
DROP FUNCTION IF EXISTS check_workspace_insight_limit();
...
DROP FUNCTION IF EXISTS update_goal_progress_on_task_completion();
...
SELECT 
    ...
    ROUND((wg.current_value / wg.target_value * 100), 1) as goal_completion_pct,
...
SELECT 
    ...
    wg.target_value,
    wg.current_value,
    ROUND((wg.current_value / wg.target_value * 100), 1) as completion_percentage,
    ...
    CASE 
        WHEN wg.last_validation_at IS NULL THEN true
        WHEN wg.last_validation_at < NOW() - INTERVAL '1 minute' * wg.validation_frequency_minutes THEN true
        ELSE false
    END as needs_validation
...
GROUP BY wg.id, wg.workspace_id, wg.metric_type, wg.target_value, wg.current_value, 
         wg.last_validation_at, wg.validation_frequency_minutes;
...
-- ============================================
-- 🎉 SETUP COMPLETE! Test the setup
-- ============================================

-- Test query to verify all tables are ready
SELECT 
    'workspace_goals' as table_name, 
    COUNT(*) as row_count,
    EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'workspace_goals') as exists
FROM workspace_goals
UNION ALL
SELECT 
    'workspace_insights' as table_name, 
    COUNT(*) as row_count,
    EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'workspace_insights') as exists
FROM workspace_insights
UNION ALL
SELECT 
    'tasks (goal fields)' as table_name,
    COUNT(*) FILTER (WHERE goal_id IS NOT NULL) as goal_linked_tasks,
    EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name = 'tasks' AND column_name = 'goal_id') as has_goal_columns
FROM tasks;