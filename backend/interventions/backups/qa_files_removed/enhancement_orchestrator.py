...
from backend.ai_quality_assurance.quality_validator import QualityAssessment, QualityIssueType
from backend.ai_quality_assurance.ai_evaluator import EnhancedAIQualityValidator
...
class AssetEnhancementOrchestrator:
    ...
    def __init__(self):
        ...
        logger.info(
            "🔧 AssetEnhancementOrchestrator initialized with Enhanced AI Quality Validator"
        )
    ...
    async def analyze_and_enhance_deliverable_assets(
        self, workspace_id: str, actionable_deliverable: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"🔍 QUALITY ANALYSIS: Starting orchestration #{self.orchestration_count} for workspace {workspace_id}"
            )

            # Validazione input
            if not actionable_deliverable or not isinstance(
            ...
            workspace_goal = actionable_deliverable.get("meta", {}).get(
                "project_goal", ""
            )

            if not actionable_assets:
                logger.warning(
                    f"No actionable assets found in deliverable for {workspace_id}"
                )
            ...
            for asset_id, asset_obj in actionable_assets.items():
                try:
                    ...
                    if quality_assessment.needs_enhancement:
                        ...
                        logger.warning(
                            f"🔧 NEEDS ENHANCEMENT: {asset_name} "
                            f"(score: {quality_assessment.overall_score:.2f}, "
                            f"issues: {len(quality_assessment.quality_issues)})"
                        )
                    else:
                        logger.info(
                            f"✅ QUALITY OK: {asset_name} "
                            f"(score: {quality_assessment.overall_score:.2f})"
                        )

                except Exception as e:
                    logger.error(
                        f"❌ Error analyzing asset {asset_id}: {e}", exc_info=True
                    )
                    # Crea assessment fallback
                    quality_reports[asset_id] = self._create_fallback_assessment(
                        asset_id, str(e)
                    )
                    continue
            ...
            if assets_needing_enhancement:
                logger.info(
                    f"🚀 ORCHESTRATING: {len(assets_needing_enhancement)} assets need improvement"
                )
            ...
            logger.info(
                f"🔍 QUALITY ANALYSIS COMPLETE: {workspace_id} ({execution_time:.1f}s)"
            )
            logger.info(f"  ├─ Assets analyzed: {len(actionable_assets)}")
            logger.info(
                f"  ├─ High quality: {len([q for q in quality_reports.values() if not q.needs_enhancement])}"
            )
    ...
    async def _analyze_single_asset_quality(
        self, asset_id: str, asset_obj: Dict, workspace_goal: str, workspace_id: str
    ) -> QualityAssessment:
            ...
            try:
                ...
                if not asset_data or not isinstance(asset_data, dict):
                    ...
                    return self._create_fallback_assessment(
                        asset_id, "Invalid asset data structure"
                    )
                ...
                logger.warning(
                    f"Quality analysis attempt {attempt + 1} failed for {asset_id}: {e}"
                )
                if attempt == max_retries - 1:
                    logger.error(f"All attempts failed for asset {asset_id}")
                    return self._create_fallback_assessment(
                        asset_id, f"Analysis failed: {e}"
                    )
    ...
    def _create_fallback_assessment(
        self, asset_id: str, error_reason: str
    ) -> QualityAssessment:
    ...
    async def _orchestrate_asset_enhancement_robust(
        ...
        try:
            ...
            if not pm_agent:
                logger.error(
                    f"❌ CRITICAL: No Project Manager found for enhancement in {workspace_id}"
                )
            ...
            if pm_task_id:
                ...
                logger.info(
                    f"✅ PM Enhancement coordination task created: {pm_task_id}"
                )
            else:
            ...
            priority_assets = [
                asset
                for asset in assets_needing_enhancement
                if asset["quality_assessment"].enhancement_priority
                in ["critical", "high"]
            ]

            logger.info(
                f"🎯 Creating enhancement tasks for {len(priority_assets)} priority assets"
            )

            for asset_info in priority_assets:
                try:
                    enhancement_task_id = (
                        await self._create_asset_enhancement_task_robust(
                    ...
                    if enhancement_task_id:
                        ...
                        enhancement_results[f"enhancement_{asset_id}"] = (
                            enhancement_task_id
                        )
                        logger.info(
                            f"✅ Enhancement task created for {asset_id}: {enhancement_task_id}"
                        )
                    else:
                        logger.warning(
                            f"⚠️ Failed to create enhancement task for {asset_info['asset_id']}"
                        )

                except Exception as e:
                    logger.error(
                        f"❌ Error creating enhancement task for {asset_info['asset_id']}: {e}",
                        exc_info=True,
                    )
                    continue
            ...
            logger.error(
                f"❌ CRITICAL ERROR in enhancement orchestration: {e}", exc_info=True
            )
            return enhancement_results
        ...
        try:
            ...
            for agent in available_agents:
                ...
                if any(keyword in role_lower for keyword in management_keywords):
                    logger.info(
                        f"🔧 Using management role as PM: {agent['name']} ({agent['role']})"
                    )
                    return agent
            ...
            if senior_agents:
                ...
                logger.warning(
                    f"⚠️ Using senior agent as PM fallback: {selected_agent['name']}"
                )
    ...
    def _estimate_enhancement_effort(
        self, asset_name: str, quality_assessment: QualityAssessment
    ) -> float:
        ...
        priority_multiplier = priority_multipliers.get(
            quality_assessment.enhancement_priority, 1.0
        )
        ...
        quality_multiplier = (
            2.0 - quality_assessment.overall_score
        )  # Lower score = more effort

        # Calcola effort finale
        final_effort = (
            base_effort * priority_multiplier * issue_multiplier * quality_multiplier
        )
    ...
    def _determine_enhancement_strategy_robust(
        self, priority_dist: Dict, issue_dist: Dict, total_assets: int
    ) -> str:
    ...
    def _analyze_deliverable_context(
        self, actionable_deliverable: Dict
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_actionability_score": actionable_deliverable.get(
    ...
    def _determine_urgency_level(self, priority_dist: Dict, issue_dist: Dict) -> str:
        ...
        if priority_dist.get("critical", 0) > 0:
            return "urgent"
        elif priority_dist.get("high", 0) > priority_dist.get(
            "medium", 0
        ) + priority_dist.get("low", 0):
            return "high"
        elif (
            issue_dist.get("fake_content", 0) > 0
            or issue_dist.get("placeholder_data", 0) > 0
        ):
            return "medium"
        else:
            return "low"
    ...
    def _estimate_resource_requirements(
        self, assets_needing_enhancement: List[Dict]
    ) -> Dict[str, Any]:
        ...
        for asset_info in assets_needing_enhancement:
            ...
            if asset_type in self.asset_expertise_mapping:
                primary_roles = self.asset_expertise_mapping[asset_type][
                    "primary_roles"
                ]
        ...
        return {
            ...
            "estimated_specialists_needed": min(
                estimated_specialists_needed, len(required_roles)
            ),
            "coordination_complexity": (
                "high"
                if len(required_roles) > 3
                else "medium" if len(required_roles) > 1 else "low"
            ),
        }
    ...
    async def _create_pm_coordination_task_robust(
        ...
        try:
                ...
                quality_summary.append(
                    {
                        ...
                        "estimated_effort": self._estimate_enhancement_effort(
                            asset_name, quality_assessment
                        ),
                        "specialist_needed": self._get_specialist_needed_for_asset(
                            asset_name
                        ),
                    }
                )
            ...
            context_data = {
                ...
                "quality_analysis_results": {
                    ...
                    "critical_assets": len(
                        [
                            a
                            for a in assets_needing_enhancement
                            if a["quality_assessment"].enhancement_priority
                            == "critical"
                        ]
                    ),
            ...
            urgency_indicator = (
                "🚨 URGENT" if overall_plan["urgency_level"] == "urgent" else "🔧"
            )
            ...
            task_priority = self._determine_task_priority_from_urgency(
                overall_plan["urgency_level"]
            )
            ...
            if created_task and created_task.get("id"):
                logger.info(
                    f"✅ PM coordination task created: {created_task['id']} "
                    f"(Priority: {task_priority}, Effort: {overall_plan['estimated_effort_hours']:.1f}h)"
                )
                return created_task["id"]
            else:
                logger.error(
                    "❌ Database returned invalid response for PM task creation"
                )
                return None
    ...
    def _create_enhanced_pm_task_description(
        self, quality_summary: List[Dict], overall_plan: Dict, workspace_id: str
    ) -> str:
        """ENHANCED: Crea descrizione task PM più dettagliata"""

        urgency_level = overall_plan.get("urgency_level", "medium")
        enhancement_strategy = overall_plan.get(
            "enhancement_strategy", "comprehensive_improvement"
        )
...
**ORCHESTRATION**: AI-guided enhancement process (Production v1.0)"""
    ...
    def _format_detailed_quality_summary_for_pm(
        self, quality_summary: List[Dict]
    ) -> str:
        ...
        for i, asset in enumerate(quality_summary, 1):
            ...
            lines.append(
                f"{i}. **{asset['asset_name']}** (Score: {asset['quality_score']:.2f}/1.0)"
            )
            lines.append(f"   ├─ Priority: {asset['priority'].upper()}")
            lines.append(
                f"   ├─ Effort: {effort_str} | Specialist: {asset['specialist_needed']}"
            )
    ...
    def _format_strategy_details(self, strategy: str, overall_plan: Dict) -> str:
        ...
        description = strategy_descriptions.get(
            strategy, "Standard enhancement approach"
        )
    ...
    async def _create_asset_enhancement_task_robust(
        self, workspace_id: str, asset_info: Dict, workspace_goal: str
    ) -> Optional[str]:
        ...
        try:
            ...
            if not asset_data:
                logger.warning(
                    f"No asset data for {asset_name}, skipping enhancement task creation"
                )
                return None
            ...
            context_data = {
                ...
                "quality_issues": [
                    issue.value for issue in quality_assessment.quality_issues
                ],
            ...
            created_task = await create_task(
                ...
                status=TaskStatus.PENDING.value,
                priority=(
                    quality_assessment.enhancement_priority
                    if quality_assessment.enhancement_priority != "critical"
                    else "high"
                ),
                context_data=context_data,
            ...
            if created_task and created_task.get("id"):
                logger.info(
                    f"✅ Enhancement task created: {created_task['id']} "
                    f"for {asset_name} → {target_agent['name']}"
                )
                return created_task["id"]
            else:
                logger.error(
                    f"❌ Database error creating enhancement task for {asset_name}"
                )
                return None

        except Exception as e:
            logger.error(
                f"❌ Error creating asset enhancement task: {e}", exc_info=True
            )
            return None
    ...
    def _create_enhanced_asset_task_description(
        ...
        issues_section = self._format_quality_issues_for_specialist_enhanced(
            quality_assessment
        )
...
**IMPORTANT**: This is asset enhancement, not creation. Improve the existing structure and content to business-ready standards."""
    ...
    def _create_asset_sample_for_task(
        self, asset_data: Dict, max_length: int = 800
    ) -> str:
    ...
    def _format_quality_issues_for_specialist_enhanced(
        ...
        for issue in quality_assessment.quality_issues:
            issue_detail = quality_assessment.issue_details.get(
                issue.value, "No details provided"
            )
    ...
    async def _find_appropriate_specialist_robust(
        ...
        try:
            ...
            if candidates:
                ...
                logger.info(
                    f"✅ Specialist selection for {asset_name}: "
    ...
    def _add_quality_metadata(
        ...
        avg_quality = (
            sum(q.overall_score for q in quality_reports.values()) / total_assets
        )
        avg_actionability = (
            sum(q.actionability_score for q in quality_reports.values()) / total_assets
        )
        ...
        actionable_deliverable["meta"]["quality_analysis"] = {
            ...
            "quality_distribution": {
                "excellent": sum(
                    1 for q in quality_reports.values() if q.overall_score >= 0.8
                ),
                "good": sum(
                    1 for q in quality_reports.values() if 0.6 <= q.overall_score < 0.8
                ),
                "poor": sum(
                    1 for q in quality_reports.values() if q.overall_score < 0.6
                ),
            },
        ...
        actionable_deliverable["executive_summary"] = (
            original_summary + quality_addendum
        )
    ...
    def _create_quality_summary_addendum(
        ...
        quality_level = (
            "excellent"
            if avg_quality >= 0.8
            else "good" if avg_quality >= 0.6 else "needs improvement"
        )
...
{ready_assets}/{total_assets} assets are immediately ready for business use (average quality: {avg_quality:.1f}/1.0 - {quality_level}). """
    ...
    def _add_error_metadata(
        self, actionable_deliverable: Dict[str, Any], error_message: str
    ) -> Dict[str, Any]:
