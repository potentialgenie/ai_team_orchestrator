...
def update_cost_intelligence_model_costs():
    ...
    # Add gpt-4o-mini to model_costs if not present
    if '"gpt-4o-mini"' not in content:
        # Find the model_costs dictionary
        model_costs_pattern = r'(self\.model_costs\s*=\s*\{[^}]+)"gpt-3\.5-turbo":\s*\{"input":\s*[\d.]+,\s*"output":\s*[\d.]+\}'
        
        # Add gpt-4o-mini entry if not exists
        new_entry = '"gpt-4o-mini": {"input": 0.000150, "output": 0.000600}'
        
        # Update the content
        if 'self.model_costs' in content and '"gpt-4o-mini"' not in content:
            # Find position after gpt-3.5-turbo entry
            lines = content.split('\n')
            for i, line in enumerate(lines):
                if '"gpt-3.5-turbo"' in line and 'input' in line:
                    # Insert after this line
                    lines.insert(i + 1, f'            {new_entry},')
                    content = '\n'.join(lines)
...
def verify_upgrade():
    ...
    if env_file.exists():
        ...
        if 'gpt-3.5-turbo' in content.lower():
            issues.append(".env still contains gpt-3.5-turbo references")
        
        if 'DEFAULT_AI_MODEL=gpt-4o-mini' in content:
            print("✅ .env: DEFAULT_AI_MODEL set to gpt-4o-mini")
        else:
            issues.append(".env: DEFAULT_AI_MODEL not set to gpt-4o-mini")
    ...
    for file_path in files_to_check:
        if Path(file_path).exists():
            ...
            if gpt4o_count > 0:
                print(f"✅ {file_path}: {gpt4o_count} references to gpt-4o-mini")
...
def main():
    ...
    print("=" * 60)
    print("🚀 SMART MODEL UPGRADE: GPT-3.5-turbo → GPT-4o-mini")
    print("=" * 60)
    print()
    ...
    print("This will update all configuration to use GPT-4o-mini.")
    ...
    print("  2. Restore Python files: for f in *.gpt35backup; do mv $f ${f%.gpt35backup}; done")
