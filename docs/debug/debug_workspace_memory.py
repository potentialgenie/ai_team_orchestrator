...
async def debug_workspace_memory():
    ...
    try:
        ...
        # Test query_insights for learnings
        print("\n🔍 Testing query_insights for learnings...")
        try:
            ...
            print(f"✅ Learnings query result:")
            print(f"  - Found {len(learnings_response.insights)} insights")
            for insight in learnings_response.insights[:3]:
                print(f"    - {insight.insight_type.value}: {insight.content[:100]}...")
        except Exception as e:
        ...
        # Test query_insights for discoveries
        print("\n🔍 Testing query_insights for discoveries...")
        try:
            ...
            print(f"✅ Discoveries query result:")
            print(f"  - Found {len(discoveries_response.insights)} insights")
            for insight in discoveries_response.insights[:3]:
                print(f"    - {insight.insight_type.value}: {insight.content[:100]}...")
        except Exception as e:
            print(f"❌ Discoveries query failed: {e}")
            import traceback
            traceback.print_exc()
