...
function shouldShowPopup(trigger = 'unknown') {
    ...
    // BUT: Allow fresh sessions (like incognito) to bypass this cooldown
    const lastShownTime = localStorage.getItem('lastPopupShownAt');
    const hasSessionData = sessionStorage.getItem('sessionInitialized');
    
    if (lastShownTime && hasSessionData) {
        // Only apply cooldown if this is a continuation of an existing session
    ...
    } else if (lastShownTime && !hasSessionData) {
        // Fresh session (incognito, new browser tab, etc.) - reduce cooldown to 1 hour
        const timeSinceLastShown = Date.now() - parseInt(lastShownTime);
        const reducedCooldown = 60 * 60 * 1000; // 1 hour instead of 24h
        
        if (timeSinceLastShown < reducedCooldown) {
            const remainingMinutes = Math.ceil((reducedCooldown - timeSinceLastShown) / 60000);
            debugLog(`Fresh session: reduced cooldown - ${remainingMinutes} minutes remaining`);
            return false;
        } else {
            debugLog('Fresh session: cooldown expired, allowing popup');
        }
    }
...
function initLeadGeneration() {
    ...
    // Mark session as initialized (helps distinguish fresh vs continuing sessions)
    if (!sessionStorage.getItem('sessionInitialized')) {
        sessionStorage.setItem('sessionInitialized', Date.now().toString());
        debugLog('Fresh session initialized');
    } else {
        debugLog('Continuing existing session');
    }
