...
def apply_reader_tools_simple(file_path):
    """Apply reader tools to a specific file"""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Check if already processed
        if 'showBookmarks' in content:
            print(f"SKIPPED: {file_path} - already has tools")
            return True
        
        # Find the closing </style> tag and insert CSS
        css_insertion = '''        /* Reader Tools */
        .reader-tools {
            position: fixed;
            top: 50%;
            right: 20px;
        ...
        # Find insertion point for HTML (before Mermaid.js)
        html_insertion = '''        <!-- Reader Tools -->
...
'''
        ...
        # Insert before Mermaid.js
        if '<!-- Mermaid.js for diagrams -->' in content:
            content = content.replace('    <!-- Mermaid.js for diagrams -->', html_insertion + '    <!-- Mermaid.js for diagrams -->')
        else:
            # Find alternative insertion point
            content = content.replace('    </div>\n\n    <script', html_insertion + '    </div>\n\n    <script')
    ...
    <script>
        ...
        function toggleReaderTools() {
            ...
            if (readerToolsCollapsed) { tools.classList.add('collapsed'); } else { tools.classList.remove('collapsed'); }
        ...
        function toggleTheme() {
            ...
            if (isDarkMode) { body.classList.remove('dark-mode'); themeIcon.textContent = '🌙'; localStorage.setItem('theme', 'light'); } 
            else { body.classList.add('dark-mode'); themeIcon.textContent = '☀️'; localStorage.setItem('theme', 'dark'); }
        }
        function toggleBookmark() {
            ...
            if (existingIndex > -1) { bookmarks.splice(existingIndex, 1); showToast('Bookmark rimosso!', 'error'); } 
            else { bookmarks.push({ url: currentPage, title: pageTitle, timestamp: new Date().toISOString() }); showToast('Bookmark salvato!', 'success'); }
            localStorage.setItem('bookmarks', JSON.stringify(bookmarks));
        }
        function switchLanguage(lang) {
            const currentPath = window.location.pathname;
            let newPath;
            if (lang === 'en') { newPath = currentPath.replace('/it/', '/en/').replace('user-experience-trasparenza', 'user-experience-transparency').replace('esecuzione-qualita', 'execution-quality').replace('filosofia-core-architettura', 'core-philosophy-architecture'); } 
            else { newPath = currentPath.replace('/en/', '/it/').replace('user-experience-transparency', 'user-experience-trasparenza').replace('execution-quality', 'esecuzione-qualita').replace('core-philosophy-architecture', 'filosofia-core-architettura'); }
        ...
        function showBookmarks() {
            ...
            if (bookmarks.length === 0) { list.innerHTML = '<div class="empty-bookmarks"><div class="empty-bookmarks-icon">📚</div><p>Nessun bookmark salvato ancora.</p><p>Clicca sull\\'icona 🔖 per aggiungere questa pagina ai tuoi bookmark!</p></div>'; } 
            else { list.innerHTML = bookmarks.map(bookmark => `<div class="bookmark-item"><div class="bookmark-info"><a href="${bookmark.url}" class="bookmark-title">${bookmark.title}</a><div class="bookmark-date">Salvato il ${new Date(bookmark.timestamp).toLocaleDateString('it-IT')}</div></div><div class="bookmark-actions"><button class="bookmark-action" onclick="deleteBookmark('${bookmark.url}')" title="Rimuovi bookmark">🗑️</button></div></div>`).join(''); }
            modal.classList.add('show');
        }
        function closeBookmarksModal() { document.getElementById('bookmarksModal').classList.remove('show'); }
        ...
        function showFontSizeMenu() {
            ...
            let newSize;
            switch (currentSize) { case 'small': newSize = 'medium'; break; case 'medium': newSize = 'large'; break; case 'large': newSize = 'small'; break; default: newSize = 'medium'; }
            setFontSize(newSize);
        ...
        function setFontSize(size) {
            ...
            const sizes = { small: '0.9', medium: '1.0', large: '1.1' };
        ...
        function updateReadingProgress() {
            ...
            window.addEventListener('scroll', () => {
                ...
                const progress = Math.min(100, Math.max(0, ((scrollTop + windowHeight - articleTop) / articleHeight) * 100));
                progressFill.style.width = progress + '%';
            });
        }
        function loadUserPreferences() {
            const savedTheme = localStorage.getItem('theme');
            if (savedTheme === 'dark') { document.body.classList.add('dark-mode'); document.getElementById('themeIcon').textContent = '☀️'; }
            const savedFontSize = localStorage.getItem('fontSize');
            if (savedFontSize) { setFontSize(savedFontSize); }
            const toolsCollapsed = localStorage.getItem('readerToolsCollapsed') === 'true';
            if (toolsCollapsed) { document.getElementById('readerTools').classList.add('collapsed'); readerToolsCollapsed = true; }
        }
        function initializeReaderTools() {
            const currentPage = window.location.pathname;
            const bookmarks = JSON.parse(localStorage.getItem('bookmarks') || '[]');
            const isBookmarked = bookmarks.some(b => b.url === currentPage);
            if (isBookmarked) { document.querySelector('.reader-tool:nth-child(3)').classList.add('active'); }
        }
    </script>
'''
        
        # Insert before closing body tag
        content = content.replace('</body>', js_insertion + '\n</body>')
...
files_to_process = [
    "./en/execution-quality/autonomous-monitoring/index.html",
    "./en/execution-quality/comprehensive-test/index.html", 
    "./en/execution-quality/consolidation-test/index.html",
    "./en/execution-quality/final-assembly-last-mile/index.html",
    "./en/execution-quality/improvement-cycle-correction/index.html",
    "./en/execution-quality/memory-system-learning/index.html",
    "./en/execution-quality/production-test/index.html",
    "./en/execution-quality/quality-gates-human-loop/index.html",
    "./en/memory-system-scaling/epilogue-journey/index.html",
    "./en/memory-system-scaling/global-scale/index.html",
    "./en/memory-system-scaling/great-refactoring/index.html",
    "./en/memory-system-scaling/load-testing-shock/index.html",
    "./en/memory-system-scaling/memory-consolidation/index.html",
    "./en/memory-system-scaling/orchestrator-wars/index.html",
    "./en/memory-system-scaling/production-readiness-audit/index.html",
    "./en/memory-system-scaling/rate-limiting-resilience/index.html",
    "./en/memory-system-scaling/security-hardening/index.html",
    "./en/memory-system-scaling/semantic-caching/index.html",
    "./en/memory-system-scaling/service-registry/index.html",
    "./en/user-experience-transparency/contextual-chat/index.html",
    "./en/user-experience-transparency/control-room/index.html",
    "./en/user-experience-transparency/deep-reasoning/index.html",
    "./en/user-experience-transparency/onboarding-ux/index.html",
    "./en/user-experience-transparency/synthesis-abstraction/index.html",
    "./en/user-experience-transparency/tech-stack/index.html",
    "./it/esecuzione-qualita/assemblaggio-finale-ultimo-miglio/index.html",
    "./it/esecuzione-qualita/ciclo-miglioramento-auto-correzione/index.html",
    "./it/esecuzione-qualita/monitoraggio-autonomo-controllo/index.html",
    "./it/esecuzione-qualita/quality-gate-human-loop/index.html",
    "./it/esecuzione-qualita/sistema-memoria-agente-impara/index.html",
    "./it/esecuzione-qualita/test-comprensivo-esame-maturita/index.html",
    "./it/esecuzione-qualita/test-consolidamento-semplificare/index.html",
    "./it/esecuzione-qualita/test-produzione-sopravvivere/index.html",
    "./it/filosofia-core-architettura/agente-ambiente-database-shared/index.html",
    "./it/filosofia-core-architettura/parsing-contratti-ai-affidabili/index.html",
    "./it/memory-system-scaling/enterprise-security-hardening-paranoia/index.html",
    "./it/memory-system-scaling/epilogo-mvp-global-platform-viaggio/index.html",
    "./it/memory-system-scaling/global-scale-architecture-timezone/index.html",
    "./it/memory-system-scaling/grande-refactoring-universal-pipeline/index.html",
    "./it/memory-system-scaling/guerra-orchestratori-unified/index.html",
    "./it/memory-system-scaling/holistic-memory-consolidation-unificazione/index.html",
    "./it/memory-system-scaling/load-testing-shock-successo-nemico/index.html",
    "./it/memory-system-scaling/production-readiness-audit-moment-truth/index.html",
    "./it/memory-system-scaling/rate-limiting-circuit-breakers-resilienza/index.html",
    "./it/memory-system-scaling/service-registry-architecture-ecosistema/index.html",
    "./it/memory-system-scaling/sistema-caching-semantico-ottimizzazione/index.html",
    "./it/user-experience-trasparenza/deep-reasoning-scatola-nera/index.html",
    "./it/user-experience-trasparenza/sala-controllo-monitoring-telemetria/index.html",
    "./it/user-experience-trasparenza/sintesi-astrazione-funzionale/index.html",
    "./it/user-experience-trasparenza/stack-tecnologico-fondamenta/index.html",
    "./it/user-experience-trasparenza/tesi-b2b-saas-versatilita/index.html"
]
...
for file_path in files_to_process:
    if apply_reader_tools_simple(file_path):
        success_count += 1
